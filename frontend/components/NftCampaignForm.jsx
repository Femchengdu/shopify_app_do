...
const NO_DISCOUNT_OPTION = { label: "No discount {-_-}/", value: "" };
...
export function NftCampaignForm({ NFTCampaign: InitialNFTCampaign }) {
  const [NFTCampaign, setNFTCampaign] = useState(InitialNFTCampaign);
  const [showResourcePicker, setShowResourcePicker] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState(NFTCampaign?.product);
  const navigate = useNavigate();
  const appBridge = useAppBridge();
  const fetch = useAuthenticatedFetch();
  const deletedProduct = NFTCampaign?.product?.title === "Deleted product";
  ...
  */
  const {
    ...
    fields: {
      title: useField({
        value: NFTCampaign?.title || "",
        validates: [notEmptyString("Please name your QR code")],
      }),
      productId: useField({
        value: deletedProduct ? "Deleted product" : NFTCampaign?.product?.id || "",
        validates: [notEmptyString("Please select a product")],
      }),
      variantId: useField(NFTCampaign?.variantId || ""),
      handle: useField(NFTCampaign?.handle || ""),
      destination: useField(
        NFTCampaign?.destination ? [NFTCampaign.destination] : ["product"]
      ),
      discountId: useField(NFTCampaign?.discountId || NO_DISCOUNT_OPTION.value),
      discountCode: useField(NFTCampaign?.discountCode || ""),
    },
    onSubmit,
  });

  const NFTCampaignURL = NFTCampaign
    ? new URL(`/nftcampaigns/${NFTCampaign.id}/image`, location.toString()).toString()
    : null;
  ...
  */
  ...
  const deleteNFTCampaign = () => console.log("delete");
  ...
  */
  ...
  }, [NFTCampaign, selectedProduct, destination, discountCode, handle, variantId]);
      ...
      <Layout>
          ...
          <Form>
            ...
            <FormLayout>
              ...
              >
                ...
                <Card.Section title="Campaign details">
                  <ChoiceList
                    title="Campaign details"
                    titleHidden
                    choices={[
                      { label: "Example campaign selection 1", value: "product" },
                      {
                        label: "Example campaign selection 2",
                        value: "checkout",
                      },
                    ]}
              ...
              <Card
                sectioned
                title="Specify discount code rules"
                actions={[
                  {
                    content: "Upload discount codes",
                    onAction: () =>
                      navigate(
                        {
                          name: "Discount",
        ...
        <Layout.Section secondary>
          <Card sectioned title="NFT Campaign">
            {NFTCampaign ? (
              <EmptyState imageContained={true} image={NFTCampaignURL} />
            ) : (
              <EmptyState>
                <p>Your NFT campaign details will appear here after you save.</p>
              </EmptyState>
            )}
            <Stack vertical>
              <Button
                ...
                download
                url={NFTCampaignURL}
                disabled={!NFTCampaign || isDeleting}
              >
                Download
              </Button>
              <Button
